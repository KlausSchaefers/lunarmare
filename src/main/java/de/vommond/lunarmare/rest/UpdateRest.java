...
public class UpdateRest extends Rest implements Handler<RoutingContext>{

	private Logger logger = LoggerFactory.getLogger(UpdateRest.class);
	
	public UpdateRest(ModelService ds, Model schema, ACL acl) {
		super(ds, schema, acl);
	}
	
	public UpdateRest(ModelService ds, Model schema, ACL acl, String idParameter) {
			...
			if(isAllowed){
				...
				this.ds.update(object, res->{
					...
					if(res.succeeded()){
						 ...
						 */
						returnOk(event, this.model.getName() + ".updated");  
					} else {
